...
export async function generateMetadata({
  ...
  const t = await getTranslations<'AboutAppsheet'>({ locale: locale, namespace: 'AboutAppsheet' });
  return {
    title: t('metaTitle'),
...
export default async function AboutAppsheetPage({
  params,
}: Readonly<{ params: Promise<{ locale: string }> }>) {
  const { locale } = await params;
